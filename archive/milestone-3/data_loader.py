...
for i in range(275):  # Assuming the links are stored in a list under the key 'links'
...
for i in range(len(labels_data)):
    for j in range(len(labels_data[i])):
        ...
        if labels_data[i][j][2] > 1:
            ...
            labels_data[i][j][2] = -1
        elif labels_data[i][j][2] == 0:
            labels_data[i][j][2] = random.uniform(.01, 0.4) - random.uniform(.01, 0.4)
        ...
        if labels_data[i][j][3] > 1:
            ...
            labels_data[i][j][3] = -1
        elif labels_data[i][j][3] == 0:
            labels_data[i][j][3] = random.uniform(.01, 0.4) - random.uniform(.01, 0.4)
            

print(max_dx)
print(max_dy)
print("Avg dx abs: " + str(avg_dx_abs/num))
print("Avg dy abs: " + str(avg_dy_abs/num))
